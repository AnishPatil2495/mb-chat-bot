 ...
 */
const ChatBotContainer = ({ plugins }: { plugins?: Array<Plugin> }) => {
  // handles platform
  ...
  // handles chat window
  const { viewportHeight, viewportWidth, isChatWindowOpen } =
  ...
  const { headerButtons, chatInputButtons, footerButtons } =
    useButtonInternal();
  ...
  return (
    <>
      {shouldShowChatBot() && (
        ...
        >
          {settings.general?.showTooltip && <ChatBotTooltip />}
          <ChatBotButton />
          ...
          <div style={getChatWindowStyle()} className="rcb-chat-window">
            ...
            {settings.general?.showInputRow && (
              <ChatBotInput buttons={chatInputButtons} />
            )}
